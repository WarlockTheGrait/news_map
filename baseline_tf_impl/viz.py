...
def read_csv(csv_file):
    ...
    for index, row in enumerate(reader):
        ...
        if prev_book != book or prev_c1 != c1 or prev_c2 != c2:
            ...
            all_traj[book][c1 + ' AND ' + c2].append(np.array(row[4:], dtype='float32'))
        ...
        for rel in rmn_traj[book]:
            ...
            if min_length < len(rtraj) < 150:
                ...
                if fig_dir is None:
                    ...
                    chars = rel.split(' AND ')
                    fig_name = fig_dir + book + '__' + chars[0] + '__' + chars[1] + '.png'
                    print('figname = ', fig_name)
                    plt.savefig(fig_name)


if __name__ == '__main__':
    wmap, cmap, bmap = pickle.load(open('data/metadata.pkl', 'rb'))
    rmn_descs = read_descriptors('descriptors/descriptors.log')
    rmn_traj = read_csv('trajectories/trajectories.log')
